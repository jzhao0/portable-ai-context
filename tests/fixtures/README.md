...
`public_surface_0_1_alpha.json` is a fixed semantic inventory of the currently published alpha package/Python/CLI surface. It records installed distribution metadata, explicit `__all__` exports, and normalized argparse command/argument structure while excluding help-text formatting, callable reprs, filesystem values, secrets, and provider runtime state. Its raw SHA256 is pinned in `tests/test_public_surface_inventory.py`; changing the surface before v1 remains allowed only through an intentional reviewed inventory update.
